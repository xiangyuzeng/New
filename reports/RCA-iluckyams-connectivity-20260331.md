...
`aws-luckyus-iluckyams-rw` 实例于 2026-03-31 05:32:54 UTC 因 **AWS RDS 自动引擎小版本升级 (MySQL 8.0.42 → 8.0.44)** 而短暂关机。实例在 05:36:20 UTC 重启，升级在 05:38:19 UTC 完成。总不可用时间约 **3 分 26 秒** (从 shutdown 到 restart)。此次中断属于 **计划维护行为**，非故障性宕机。当前实例已完全恢复，连接正常。
...
| 05:36:20 | 13:36:20 | **DB 实例重启完成** | RDS Events (availability) |
...
| 05:38:48 | 13:38:48 | **VIP 不通报警恢复** | 报警系统 |
...
AWS 在维护窗口内对 `aws-luckyus-iluckyams-rw` 执行了 **MySQL 8.0.42 → 8.0.44 自动小版本升级**。此升级导致实例短暂关机并重启，触发了 VIP 不通报警。
...
**⚠️ 会。** AWS RDS 自动小版本升级是默认启用的。除非显式关闭 `Auto Minor Version Upgrade`，否则每次 AWS 发布新的 MySQL 小版本补丁时，该实例会在维护窗口内自动升级并重启。
...
4. **Review 维护窗口设置**
   - 确认当前维护窗口是否在业务低峰期
   - 考虑是否需要关闭 Auto Minor Version Upgrade，改为手动控制升级时机
   - 建议在维护窗口升级前增加通知机制
